/**
 * Hierarchical Municipality/City Page
 * URL structure: /taxi/[regionSlug]/[districtSlug]/[municipalitySlug]
 * Example: /taxi/banskobystricky-kraj/zvolen/babina
 */
...
import { getMunicipalityBySlug, findNearestCitiesWithTaxis, allMunicipalities } from '@/data/municipalities';
import { getDistrictBySlugAndRegion, getAllDistricts } from '@/data/districts';
...
export function generateStaticParams() {
  ...
  const params: Array<{ regionSlug: string; districtSlug: string; municipalitySlug: string }> = [];

  allDistricts.forEach((district) => {
    const municipalities = allMunicipalities.filter((m) => m.district === district.name);
...
export async function generateMetadata({
  ...
  params: Promise<{ regionSlug: string; districtSlug: string; municipalitySlug: string }>;
}): Promise<Metadata> {
  const { regionSlug, districtSlug, municipalitySlug } = await params;
  ...
  const city = getCityBySlug(municipalitySlug);

  if (city) {
    const siteName = 'Taxi NearMe';
    const baseUrl = 'https://www.taxinearme.sk';
    const currentUrl = `${baseUrl}/taxi/${regionSlug}/${districtSlug}/${municipalitySlug}`;
    ...
    const description =
      ...
      `Taxi v meste ${city.name}, okres ${district.name} - Kontakty na taxislužby. ${taxiServicesList ? `${taxiServicesList} a ďalšie.` : ''} Nájdite spoľahlivé taxi.`;

    return {
      title: `Taxi ${city.name} - Okres ${district.name}, ${regionName} | ${siteName}`,
      description,
  ...
  if (municipality) {
    ...
    const description = `Taxi v obci ${municipality.name}, okres ${district.name} - Najbližšie taxislužby sú v meste ${nearestCity.city.name} (${nearestCity.distance} km). Nájdite spoľahlivé taxi služby v okolí.`;

    return {
      title: `Taxi ${municipality.name} - Okres ${district.name}, ${regionName} | ${siteName}`,
      description,
      keywords: [
 ...
 */
function renderMunicipalityPage(
  municipality: {
    name: string;
    region: string;
    district: string;
    ...
    <div className="min-h-screen bg-white">
      ...
      <SEOBreadcrumbs
        items={[
          ...
          { label: `Okres ${districtName}`, href: `/taxi/${regionSlug}/${districtSlug}` },
          ...
          <div className="text-center mb-8 md:mb-12 rounded-xl md:rounded-2xl overflow-hidden relative p-6 md:p-10 lg:p-12">
            ...
            <div className="relative z-10">
              ...
              <p className="text-base md:text-xl font-semibold px-4 text-foreground/90">
                Okres {districtName}, {municipality.region}
              </p>
            </div>
          </div>
        </div>
      ...
      {nearestCities.length > 0 &&
        nearestCities[0].city.latitude &&
        nearestCities[0].city.longitude && (
            ...
            <div className="container mx-auto max-w-4xl">
              <div className="mb-6 text-center">
                ...
                <p className="text-sm md:text-base text-foreground/70 font-semibold">
                  {municipality.name} → {nearestCities[0].city.name} ({nearestCities[0].distance}{' '}
                  km)
                </p>
              </div>
              <RouteMapWrapper
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
          <div className="mb-8 text-center">
            ...
            <p className="text-base md:text-lg text-foreground/80 font-semibold">
              V obci {municipality.name} momentálne neevidujeme taxislužby. Nižšie nájdete
              najbližšie dostupné taxi služby z okolitých miest.
            </p>
          </div>
                ...
                <div className="card-3d shadow-3d-sm">
                  ...
                  <CardContent>
                    <p className="text-sm md:text-base text-foreground/70 mb-4">
                      Vzdialenosť: <strong>{distance} km</strong> | Orientačná cena:{' '}
                      <strong>
                        cca {Math.ceil(distance * 0.9)}€ - {Math.ceil(distance * 1.1)}€
                      </strong>
                    </p>
...
export default async function HierarchicalMunicipalityPage({
  params,
}: {
  params: Promise<{ regionSlug: string; districtSlug: string; municipalitySlug: string }>;
}) {
  ...
  if (city) {
    // Render main city page with taxi services
    return (
      <div className="min-h-screen bg-white">
        ...
        <SEOBreadcrumbs
          items={[
            ...
            { label: `Okres ${district.name}`, href: `/taxi/${regionSlug}/${districtSlug}` },
            { label: city.name },
          ]}
        />
        ...
        <section className="pt-4 md:pt-6 pb-8 md:pb-12 px-4 md:px-8 relative bg-white">
          ...
          <div className="container mx-auto max-w-4xl relative z-10">
            ...
            >
              ...
              {!city.heroImage && <div className="absolute inset-0 hero-3d-bg" />}

              <div className="relative z-10">
                <h1
                  className={`text-3xl md:text-5xl lg:text-6xl font-extrabold mb-3 md:mb-6 drop-shadow-lg ${city.heroImage ? 'text-white' : 'text-foreground'}`}
                >
                  Taxislužby v meste {city.name}
                </h1>
                <p
                  className={`text-base md:text-xl font-semibold px-4 ${city.heroImage ? 'text-white/95' : 'text-foreground/90'}`}
                >
                  Okres {district.name}, {regionName}
                </p>
              </div>
            </div>
          </div>
                ...
                {[...city.taxiServices]
                  ...
                  .map((service, index) => {
                    ...
                    return (
                      <Card
                        key={index}
                        className={`perspective-1000 ${service.isPremium ? 'ring-2 ring-yellow-400' : ''}`}
                      >
                        <Link
                          href={`/taxi/${city.slug}/${serviceSlug}`}
                          title={`${service.name} - Detailné informácie a kontakt`}
                        >
                          <div
                          ...
                          >
                            ...
                            <CardHeader className="pb-1 pt-3 md:pt-3.5 px-3 md:px-4">
                              <CardTitle
                                className={`text-sm md:text-base font-semibold flex items-center gap-1.5 md:gap-2 ${service.isPremium ? 'text-white' : ''}`}
                              >
                                <MapPin
                                  className={`h-3.5 w-3.5 md:h-4 md:w-4 flex-shrink-0 ${service.isPremium ? 'text-yellow-300' : 'text-success'}`}
                                />
                                {service.name}
                              </CardTitle>
                            </CardHeader>
                            <CardContent className="pt-0 pb-3 md:pb-3.5 px-3 md:px-4">
                              <div className="flex flex-col gap-1 md:gap-1.5 text-xs md:text-sm">
                                {service.phone && (
                                  <div
                                    className={`flex items-center gap-1.5 md:gap-2 font-medium ${service.isPremium ? 'text-white' : 'text-foreground'}`}
                                  >
                                    <Phone
                                      className={`h-3 w-3 md:h-3.5 md:w-3.5 flex-shrink-0 ${service.isPremium ? 'text-yellow-300' : 'text-primary-yellow'}`}
                                    />
                                    {service.phone}
                                  </div>
                                )}
                                {service.website && (
                                  ...
                                  >
                                    <Globe
                                      className={`h-3 w-3 md:h-3.5 md:w-3.5 flex-shrink-0 ${service.isPremium ? 'text-yellow-300' : 'text-info'}`}
                                    />
                  ...
                  <CardContent className="py-8 md:py-12 px-4">
                    <div className="text-center space-y-3 md:space-y-4">
                      ...
                      <p className="text-sm md:text-base text-foreground/70 font-bold px-4">
                        Čoskoro tu nájdete kompletný prehľad všetkých taxislužieb v meste{' '}
                        {city.name}
                      </p>
                    </div>
                  </CardContent>
