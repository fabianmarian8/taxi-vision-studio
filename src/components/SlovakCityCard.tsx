  ...
  return (
    <div className="perspective-1000">
      <Link href={`/taxi/${slug}`} className="block" title={`Taxi ${name} - telefónne čísla a informácie`}>
        <div className={`group relative bg-card rounded-xl md:rounded-2xl p-4 md:p-6 shadow-3d-md hover:shadow-3d-lg card-3d cursor-pointer overflow-hidden transition-all duration-300 border-l-4 ${borderColor}`}>
          ...
          <div className="relative z-10 flex items-start gap-3 md:gap-4">
            <div className={`flex-shrink-0 w-10 h-10 md:w-12 md:h-12 ${accentColor} rounded-lg md:rounded-xl flex items-center justify-center shadow-3d-sm group-hover:scale-110 transition-transform duration-300`}>
              <MapPin className="h-5 w-5 md:h-6 md:w-6 text-white" strokeWidth={2.5} />
            </div>

            <div className="flex-1 min-w-0">
              <h3 className="text-base md:text-lg lg:text-xl font-semibold text-foreground mb-1 group-hover:translate-x-1 transition-transform duration-300">
                {name}
              </h3>
