  ...
  return (
    <div className="perspective-1000">
      <Link href={`/kraj/${slug}`} className="block" title={`Taxislužby v kraji ${name}`}>
        <div className={`group relative bg-card rounded-lg md:rounded-xl p-2 md:p-4 shadow-3d-md hover:shadow-3d-lg card-3d cursor-pointer overflow-hidden transition-all duration-300 border-l-4 ${borderColor}`}>
          ...
          <div className="relative z-10 flex items-start gap-2 md:gap-2">
            <div className={`flex-shrink-0 w-6 h-6 md:w-7 md:h-7 ${accentColor} rounded-md md:rounded-lg flex items-center justify-center shadow-3d-sm group-hover:scale-110 transition-transform duration-300`}>
              <MapPin className="h-3 w-3 md:h-4 md:w-4 text-white" strokeWidth={2.5} />
            </div>

            <div className="flex-1 min-w-0">
              <h3 className="text-xs md:text-sm lg:text-base font-semibold text-foreground mb-1 group-hover:translate-x-1 transition-transform duration-300">
                {name}
              </h3>
