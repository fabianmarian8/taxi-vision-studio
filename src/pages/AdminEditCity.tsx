...
export default function AdminEditCity() {
    ...
    if (suggestionsParam) {
      try {
        ...
        toast({
          ...
          description: `Pridaných ${newServices.length} návrhov. Skontrolujte a kliknite na "Uložiť zmeny".`,
        });

        // odstráň query parametre, aby sa nepridali znova
  ...
  }, [city, taxiServices.length]); // zámerne viazané na dĺžku, aby sa necyklilo

  // Varovanie pri odchode s neuloženými zmenami
  useEffect(() => {
  ...
  const loadCityData = async () => {
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        if (!foundCity) {
          toast({ title: 'Chyba', description: 'Mesto nebolo nájdené', variant: 'destructive' });
  ...
  // JEDNO tlačidlo: uložiť + publikovať + označiť suggestions
  const handleSaveChanges = async () => {
    if (!hasUnsavedChanges) {
      toast({ title: 'Info', description: 'Žiadne zmeny na uloženie' });
      return;
    }
    ...
    try {
      ...
      if (!(publishRes.status >= 200 && publishRes.status < 300)) {
        ...
        throw new Error((errData as any).message || 'Nepodarilo sa publikovať zmeny');
      }

      // Označ suggestions ako schválené
      const pendingIds = sessionStorage.getItem('pendingSuggestionIds');
      if (pendingIds) {
        await fetch('/api/suggestions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ action: 'mark-approved', suggestionIds: pendingIds.split(',') }),
        }).catch(() => {});
      ...
      toast({
        title: '✅ Uložené a publikované',
        description: 'Zmeny boli uložené a publikované. Deployment prebehne automaticky.',
      });
    } catch (e) {
      toast({
        title: 'Chyba',
        description:
          e instanceof Error ? e.message : 'Nepodarilo sa uložiť a publikovať',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  const handleFindNewServices = async () => {
    ...
    try {
      const scraperResponse = await fetch('/api/gbp-scraper', {
        ...
        body: JSON.stringify({ city: city.name, limit: 15 }),
      });
      const scraperData = await scraperResponse.json();

      if (!(scraperResponse.status >= 200 && scraperResponse.status < 300)) {
        const errorMessage = scraperData.message || scraperData.error || 'Neznáma chyba API';
        toast({ title: 'Chyba API', description: errorMessage, variant: 'destructive' });
        setIsScrapingInProgress(false);
        return;
      }

      if (!scraperData.success || scraperData.count === 0) {
        toast({ title: 'Informácia', description: 'Nenašli sa žiadne nové taxislužby' });
        setIsScrapingInProgress(false);
        return;
      }
      ...
      if (!(suggestionsResponse.status >= 200 && suggestionsResponse.status < 300)) {
        const suggestionsData = await suggestionsResponse.json().catch(() => ({}));
        const errorMessage =
          (suggestionsData as any).message ||
          (suggestionsData as any).error ||
          'Nepodarilo sa uložiť návrhy';
      ...
      if (result.added === 0) {
        toast({
          title: 'Informácia',
          description: `Našlo sa ${scraperData.count} výsledkov, ale všetky sú už v databáze (${result.skipped} duplicít).`,
        });
      } else {
        toast({
          title: 'Úspech',
      ...
      toast({
        title: 'Chyba',
        description:
          error instanceof Error ? error.message : 'Nepodarilo sa nájsť nové taxislužby',
        variant: 'destructive',
      });
    } finally {
      setIsScrapingInProgress(false);
            ...
            <AlertDescription className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Zmeny sú uložené len lokálne. Kliknite na "Uložiť zmeny".
                </p>
              </div>
            </AlertDescription>
          </Alert>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                ...
                <li>• Keď ste hotový, kliknite na "Uložiť zmeny"</li>
                <li>• Zmeny sa automaticky publikujú a spustí sa nový deploy</li>
              </ul>
            </div>
          </CardContent>
        </Card>
