...
export default function AdminEditCity() {
  ...
  useEffect(() => {
    ...
    if (suggestionsParam) {
      try {
        ...
        toast({ title: '✅ Návrhy pridané lokálne', description: `Pridaných ${newServices.length} návrhov.` });
  ...
  const handleFindNewServices = async () => {
    ...
    } catch (error) { toast({ title: 'Chyba', description: error instanceof Error ? error.message : 'Nepodarilo sa nájsť nové taxislužby', variant: 'destructive' }); }
