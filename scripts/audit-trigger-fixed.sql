-- ===========================================
-- VYLEPŠENÝ AUDIT TRIGGER pre partner portal aktivity
-- ===========================================
...
-- 3. RPC funkcia pre nastavenie audit contextu (session variables)
...
AS $$
BEGIN
  -- Nastaví session variables, ktoré trigger môže čítať
...
-- 4. Helper funkcia pre získanie user info (teraz z session variables alebo auth)
...
DECLARE
  ...
  v_ip_address text;
BEGIN
  -- Najprv skús session variables (nastavené cez set_audit_context)
  v_user_id := NULLIF(current_setting('audit.user_id', true), '')::uuid;
  v_user_email := NULLIF(current_setting('audit.user_email', true), '');
  v_ip_address := NULLIF(current_setting('audit.ip_address', true), '');

  -- Ak nie sú session variables, skús auth kontext (pre priame RLS operácie)
  IF v_user_id IS NULL THEN
    v_user_id := auth.uid();
    v_user_email := (auth.jwt()->>'email')::text;
    v_ip_address := (current_setting('request.headers', true)::json->>'x-forwarded-for')::text;
  END IF;

  RETURN QUERY
  SELECT v_user_id, v_user_email, v_ip_address;
END;
$$;

-- 5. Hlavná trigger funkcia (nezmenená)
...
-- 6. RPC funkcia pre logovanie partner prihlásení
CREATE OR REPLACE FUNCTION log_partner_login(
  p_user_id uuid,
  p_user_email text,
  p_ip_address text DEFAULT NULL
)
...
BEGIN
  INSERT INTO audit.activity_log (
    ...
    'auth', 'sessions', 'LOGIN', p_user_id, p_user_email,
    p_ip_address, NULL, NULL, NULL
  );
END;
$$;

-- 7. Funkcia pre aktiváciu trackingu na tabuľke
...
-- 8. Funkcia pre deaktiváciu trackingu
