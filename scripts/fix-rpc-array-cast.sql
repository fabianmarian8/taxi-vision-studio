...
BEGIN
  ...
  SET
    ...
    -- Pridané polia pre publish
    status = COALESCE((p_changes->>'status'), status),
    submitted_at = COALESCE((p_changes->>'submitted_at')::TIMESTAMPTZ, submitted_at),
    reviewed_at = COALESCE((p_changes->>'reviewed_at')::TIMESTAMPTZ, reviewed_at),
    updated_at = NOW()
  WHERE id = p_draft_id
...
BEGIN
  ...
  INSERT INTO partner_drafts (
    ...
    COALESCE(p_changes->>'status', 'pending'),
